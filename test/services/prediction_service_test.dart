  ...
  group('PredictionService.evaluatePoints', () {
    ...
    group('Group Stage Matches', () {
      ...
      test('Exact scoreline returns exact points', () {
        ...
        expect(PredictionService.evaluatePoints(match, pred), kExactScorePoints);
      });

      test('Correct outcome (win) but wrong scoreline returns correct outcome points', () {
        final pred = MatchPrediction(matchId: '1', t1Score: 3, t2Score: 0);
        expect(PredictionService.evaluatePoints(match, pred), kCorrectOutcomePoints);
      });

      test('Incorrect outcome returns 0 points', () {
        final pred = MatchPrediction(matchId: '1', t1Score: 1, t2Score: 2); // predicted loss
        expect(PredictionService.evaluatePoints(match, pred), 0);

        final pred2 = MatchPrediction(matchId: '1', t1Score: 1, t2Score: 1); // predicted draw
        expect(PredictionService.evaluatePoints(match, pred2), 0);
      });

      test('Correct outcome (draw) but wrong scoreline returns correct outcome points', () {
        final drawMatch = match.copyWith(t1Score: 1, t2Score: 1);
        final pred = MatchPrediction(matchId: '1', t1Score: 0, t2Score: 0);
        expect(PredictionService.evaluatePoints(drawMatch, pred), kCorrectOutcomePoints);
      });

      test('Correct outcome (loss) but wrong scoreline returns correct outcome points', () {
        final lossMatch = match.copyWith(t1Score: 0, t2Score: 2);
        final pred = MatchPrediction(matchId: '1', t1Score: 1, t2Score: 3);
        expect(PredictionService.evaluatePoints(lossMatch, pred), kCorrectOutcomePoints);
      });
    });
    ...
    group('Knockout Stage Matches (90 minutes)', () {
      ...
      test('Exact 90-min scoreline returns exact knockout points', () {
        ...
        expect(PredictionService.evaluatePoints(knockoutMatch, pred), kExactScoreKnockoutPoints);
      });

      test('Correct 90-min outcome but wrong scoreline returns correct outcome knockout points', () {
        final pred = MatchPrediction(matchId: '2', t1Score: 3, t2Score: 0);
        expect(PredictionService.evaluatePoints(knockoutMatch, pred), kCorrectOutcomeKnockoutPts);
      });
      ...
      test('Correct 90-min outcome (draw that goes to ET) but wrong scoreline returns correct outcome knockout points', () {
        ...
        expect(PredictionService.evaluatePoints(drawMatch, pred), kCorrectOutcomeKnockoutPts);
      });
    });
    ...
    group('Knockout Stage Matches (Beyond 90 minutes)', () {
      ...
      test('Correct 90-min outcome + correct extra-time winner returns outcome points + ET bonus', () {
        final pred = MatchPrediction(
          matchId: '3',
          t1Score: 0,
          t2Score: 0,
        ...
        expect(PredictionService.evaluatePoints(etMatch, pred), kCorrectOutcomeKnockoutPts + kExtraTimeBonusPoints);
      });

      test('Correct 90-min outcome + wrong extra-time winner returns only outcome points', () {
        final pred = MatchPrediction(
          matchId: '3',
          t1Score: 0,
          t2Score: 0,
        ...
        expect(PredictionService.evaluatePoints(etMatch, pred), kCorrectOutcomeKnockoutPts);
      });

      test('Correct 90-min outcome + correct penalty shootout winner returns outcome + PK bonus (+ ET bonus if applicable)', () {
        ...
        expect(PredictionService.evaluatePoints(pkMatch, pred), kCorrectOutcomeKnockoutPts + kExtraTimeBonusPoints + kPenaltyShootoutBonusPoints);
      });

      test('Correct 90-min outcome + wrong penalty shootout winner returns no PK bonus', () {
        final pred = MatchPrediction(
          matchId: '4',
          t1Score: 0,
          t2Score: 0,
        ...
        expect(PredictionService.evaluatePoints(pkMatch, pred), kCorrectOutcomeKnockoutPts + kExtraTimeBonusPoints);
      });

      test('Exact 90-min scoreline + correct ET/PK bonuses returns exact points + bonuses', () {
        final pred = MatchPrediction(
          matchId: '4',
          t1Score: 1,
        ...
        expect(PredictionService.evaluatePoints(pkMatch, pred), kExactScoreKnockoutPoints + kExtraTimeBonusPoints + kPenaltyShootoutBonusPoints);
      });

      test('Incorrect 90-min outcome but correct ET winner returns 0 points (bonuses only apply if 90-min outcome is correct)', () {
        final pred = MatchPrediction(
          matchId: '3',
